...
def get_job_table_sample(workload_file_name, num_materialized_samples = 1000):
    ...
    num_bytes_per_bitmap = int((num_materialized_samples) >> 3)
...
# def extract_filter(filter):
#     f, fs = filter, []
#     f_parens = find_parens(f)
#     while 0 in f_parens and f_parens[0] == len(f) - 1:
#         f = f[1:-1]
#         f_parens = find_parens(f)
#     try:
#         for ff in f.split(' AND '):
#             ff_parens = find_parens(ff)
#             while 0 in ff_parens and ff_parens[0] == len(ff) - 1:
#                 ff = ff[1:-1]
#                 ff_parens = find_parens(ff)
#             fs += ff.split(' OR ')
#     except:
#         for ff in f.split(' OR '):
#             ff_parens = find_parens(ff)
#             while 0 in ff_parens and ff_parens[0] == len(ff) - 1:
#                 ff = ff[1:-1]
#                 ff_parens = find_parens(ff)
...
#                 break
#         else:
#             raise Exception(f'Unrecognized op in {f}')
#         f = f.split(' '+op+' ')
#         fs[i] = [f[0], op, f[1]]
#     return fs
...
def extract_predicates(ps):
    ...
    if 'AND' in ps or 'OR' in ps:
        ...
        predicate.append([(''.join(ps[:op_pos])).rstrip(), ps[op_pos], ''.join(ps[op_pos+1:])])
    return predicate

def extract_filter(filter):
    fs = extract_predicates(expr.parse_string(filter, parseAll=True)[0].as_list())
    return fs

def extract_col(col):
    # print(f'extract_col({col})')
    if len(find_parens(col)) == 1:
        return col[col.find('.')+1:col.find(')')]
    else:
...
#        print(filt, alias)
        ...
        for filt in filters:
            ...
            # fs = filt.split(' AND ')
            try:
                fs = extract_filter(filt)
            except Exception as e:
                # print(f'Failed to extract filters from {filt}')
                continue
            for f in fs:
     #           print(filters)
                # col, op, num = f.split(' ')
    ...
    #            print(f)
                ...
                try:
                    ...
                    # print(f'Failed to get colId of {column}')
                    continue
                res['opId'].append(self.op2idx[op])
                if type(num) == str:
                    res['val'].append(self.normalize_val(column, float(0)))
                else:
                    res['val'].append(self.normalize_val(column, float(num)))
        # limit filters num
        res['colId'] = res['colId'][:3]
        res['opId'] = res['opId'][:3]
        res['val'] = res['val'][:3]
        return res
