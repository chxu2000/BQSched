...
class MaskableEvalCallback(EvalCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0 and self.n_calls != self.last_eval_n_calls:
            ...
            self.current_epoch = self.n_calls // self.eval_freq

            if mean_reward > self.best_mean_reward:
                # self.training_env.envs[0].env.env.env.baseline = -mean_reward
                if self.verbose > 0:
                    print("New best mean reward!")
                if self.best_model_save_path is not None:
                    self.last_save_epoch = self.current_epoch
                    self.model.save(os.path.join(self.best_model_save_path, f"best_model_{self.current_epoch}"))
