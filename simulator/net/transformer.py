...
class Prediction(nn.Module):
    def __init__(self, in_feature = 69, hid_units = 256, contract = 1, mid_layers = True, res_con = True, pred_type = "cls"):
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_bias=True, mask=True):
        orig_q_size = q.size()
        ...
        if attn_bias is not None:
            x = x + attn_bias
        if mask is not None:
            mask = mask.view(mask.shape[0], 1, mask.shape[1], mask.shape[2]).expand_as(x)
            x[mask] = -np.inf
